...
const CharacterImageScreen = () => {
    ...
    const { dispatch, upsertCharacterCreate, aiGenerated, reset } = useCreateCharacterSlice();
    ...
    return (
        <Screen preset="fixed" safeAreaEdges={['top']} contentContainerStyle={[$rootContainer]}>
            <View style={$contentContainer}>
                <CreateStepper totalSteps={8} currentStep={6} />
                <ScrollView
                    style={{
                        display: 'flex',
                        flexGrow: 1,
                    ...
                    <CreateLabel title={['캐릭터와 이미지를', '등록해 주세요']} />
                    <View style={$imageBox}>
                        {image != null ? (
                            <TouchableOpacity
                                onPress={() => {
