...
export class CharacterApi extends Api {
    async createCharacter(character: AddCharacterRequest) {
        try {
            await this.setClientHeaders(await getPersistData(DataPersistKeys.TOKEN));

            const response: any = await this.postMultipart('/chara/add', character);
    ...
    async updateCharacter(update: updateCharacterRequest) {
        try {
            ...
            const response: any = await this.axiosInstance.patch('/chara/update', update);
